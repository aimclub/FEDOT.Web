"""Supervision of discovery runs: spawning, progress fan-out and cancellation.

An equation search is CPU-bound and blocking, and importing EPDE pulls in torch
and scikit-learn, so a run gets its own process: the API stays responsive, the
server itself never imports the framework, and cancellation is a matter of
killing a process tree.

Progress arrives as JSON lines the worker appends to ``events.jsonl``, which the
tail below streams into storage and out to any WebSocket listeners.
"""
    ...
    def publish(self, run_uid: str, event: dict[str, Any]) -> None:
        for queue in list(self._queues.get(run_uid, ())):
            try:
                ...
                # to stall the run; the client refetches the log on reconnect.
...
class RunManager:
    ...
    async def start(self, run_uid: str, config: dict[str, Any]) -> None:
        async with self._lock:
            ...
            if sys.platform == "win32":
                # Own process group so cancellation does not signal the API server.
                creation_flags = subprocess.CREATE_NEW_PROCESS_GROUP

            process = subprocess.Popen(
                [sys.executable, "-m", "epdeweb.runs.worker", str(directory)],
                cwd=package_root,
                env=environment,
                stdout=log_file,
    ...
    async def _tail(self, run_uid: str, log_file: Any) -> None:
        ...
        try:
            while True:
                ...
                # Stop only once the worker is gone *and* its log has been
                # drained, so the final event is never lost to a race with exit.
    ...
    async def _finalise(self, run_uid: str, status: str, error: str | None) -> None:
        ...
        if status == "finished" and result_path.exists():
            ...
            if result.get("front"):
                updates["objectives"] = {
                    "names": result.get("objective_names") or [],
                    "front": [entry.get("objectives") for entry in result["front"]],
                    "generations": result.get("generations"),
                }
            best = result.get("best_system")
            if best:
                run = self._store.get_run(run_uid) or {}
                system_uid = self._store.save_system(
                    graph=best,
                    name=f"{run.get('name', 'run')} - best system",
                    origin="search",
                    run_uid=run_uid,
                )
                updates["best_system"] = system_uid

        self._store.update_run(run_uid, **updates)
        self._processes.pop(run_uid, None)
        # The tail task is about to return; dropping the reference here keeps
        # the dict from growing by one entry per run for the life of the server.
    ...
    async def cancel(self, run_uid: str) -> bool:
        ...
        # ``_tail`` notices the dead process and finalises; forcing the status
        # here means a slow poll cannot leave the run looking alive.
...
def _terminate_tree(process: subprocess.Popen) -> None:
    ...
    EPDE's preprocessors and its solver-based fitness can start worker processes
    and CUDA contexts of their own; killing only the direct child would leave
    those holding memory and the token cache open.
    """
    try:
        import psutil
        ...
        # psutil is a transitive dependency of EPDE rather than a direct one
        # here, so the plain subprocess API is a real fallback, not dead code.
        with suppress(Exception):
            process.terminate()
        with suppress(Exception):
